...
const Currency = () => {
  ...
  const handleAmountOne = (amountOne) => {
    ...
    const updatedAmountTwo = formateCurrency((amountOne * currencyRates[currencyTwo]) / currencyRates[currencyOne]);
  ...
  const handleCurrencyOne = (currencyOne) => {
    setAmountTwo(
      formateCurrency((amountOne * currencyRates[currencyTwo]) / currencyRates[currencyOne])
    );
  ...
  const handleAmountTwo = (amountTwo) => {
    setAmountOne(
      formateCurrency((amountTwo * currencyRates[currencyOne]) / currencyRates[currencyTwo])
    );
            ...
            <Container>
              <Row className="mb-3">
                <Col xs={12} md={12} className="right-side-content">
                  <p className="oneCurrency">1 {currencyOne} = {formateCurrency(amountTwo / amountOne)} {currencyTwo}</p>
                </Col>
              </Row>
              <Row>
                <Col xs={12} md={6} className="left-side-content">
                  <CurrencyConverter
                    amount={amountOne}
